...
public class UserService {
    ...
    @Transactional(readOnly = true)
    public Page<AdminUserDTO> getAllPublicUsers(PageFilterInput<AdminUserDTO> input) {
        Page<User> users = userCustomRepository.getAllUser(
            input,
            input.getPageSize() == 0 ? Pageable.unpaged() : PageRequest.of(input.getPageNumber(), input.getPageSize())
        );
        return new PageImpl<>(userMapper.usersToAdminUserDTOs(users.getContent()), users.getPageable(), users.getTotalElements());
    }
        ...
        if (existingUser.isPresent()) {
            ...
            if (details.get("updated_at") != null) {
                ...
                if (idpModifiedDate.isAfter(dbModifiedDate)) {
                    log.debug("Updating user '{}' in local database", user.getUsername());
            ...
            log.debug("Saving user '{}' in local database", user.getUsername());
